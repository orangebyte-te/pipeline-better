...
推荐模板入口 `variables/insgeek-business-ka.yml`：
...
- `DEPLOY_CONFIG_PROJECT_PATH` 明确控制 yaml-config 中的目录路径
- 当前 workflow 已直接注入 `RUNNER_TAG=dev-runner-k8s-ali`，入口文件不需要再写 `runner_tags`
