...
```yaml
include:
  - project: "legacy-templates/pipeline"
    ref: release
...
- REGISTRY（可选，不配时默认 harbor.example.internal）
- REGISTRY_USER（可选）
- REGISTRY_PASSWORD（可选）

说明
- 当前 workflow 已直接注入 `RUNNER_TAG=generic-runner-k8s`，不再依赖 `RUNNER_TAG_*` 变量
- 业务仓库 include 的 `ref` 应与模板内 `TEMPLATE_REF` 保持一致；当前验证通过的组合是 `main`
- release 分支会构建 `uat` 镜像，并同时更新 deployment-config 中的 `uat` / `pro` 目录
